...
class JSONBufferManager:
    ...
    def add_event(self, node_id: str, value: str | None, is_good: bool, status: str) -> bool:
        ...
        if not self.buffer_enabled:
            logger.error(" [JSON_БУФЕР] Буфер отключен - событие не сохранено!")
            return False
    ...
    def _rotate_file(self, file_path: Path, file_type: str):
        ...
        try:
            ...
            rotated_path = file_path.parent / f"buffer_{file_type}_{timestamp}.json"
                ...
                for rotated_file in rotated_events:
                    ...
                    try:
                        with open(rotated_file, 'r') as f:
                            data = json.load(f)
                        unsync = [r for r in data["records"] if r["synced"] == 0]
                        result["events"].extend(unsync[:limit - len(result["events"])])
                    except Exception as e:
                        logger.error(f" [JSON_БУФЕР] Ошибка чтения {rotated_file.name}: {e}")
                ...
                for rotated_file in rotated_analogs:
                    ...
                    try:
                        with open(rotated_file, 'r') as f:
                            data = json.load(f)
                        unsync = [r for r in data["records"] if r["synced"] == 0]
                        result["analogs"].extend(unsync[:limit - len(result["analogs"])])
                    except Exception as e:
                        logger.error(f" [JSON_БУФЕР] Ошибка чтения {rotated_file.name}: {e}")
    ...
    def mark_synced(self, event_ids: List[str] = None, analog_ids: List[str] = None) -> bool:
        ...
        try:
            ...
            logger.error(f" [JSON_БУФЕР] Ошибка пометки синхронизированных: {e}")
            return False
    ...
    def cleanup_synced(self) -> int:
        ...
        try:
            ...
            if not self.buffer_enabled:
                ...
                logger.info(" [JSON_БУФЕР] ✓ Буфер повторно включен после очистки")
            
            logger.info(f" [JSON_БУФЕР] Удалено {deleted_records} синхронизированных записей")
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            ...
            unsync_events = len([r for r in events_data["records"] if r["synced"] == 0])
            unsync_analogs = len([r for r in analogs_data["records"] if r["synced"] == 0])
